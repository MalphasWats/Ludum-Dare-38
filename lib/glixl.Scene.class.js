...
var glixl = (function(glixl)
{
    ...
    glixl.Scene.prototype.update = function()
    {
        ...
        {
            x1 = this.tiles[i].x+this.tiles[i].x_offset;
            y1 = this.tiles[i].y+this.tiles[i].y_offset - this.tiles[i].z;
            x2 = this.tiles[i].x+this.tiles[i].x_offset + this.tiles[i].width;
            y2 = this.tiles[i].y+this.tiles[i].y_offset + this.tiles[i].height - this.tiles[i].z;
            z = this.tiles[i].y+this.tiles[i].y_offset-(this.tiles[i].z/this.tiles[i].height);
        ...
        for(var s=0 ; s<this.sprites.length ; s++)
        {
            ...
            x1 = this.sprites[s].x+this.sprites[s].x_offset;
            y1 = this.sprites[s].y+this.sprites[s].y_offset - this.sprites[s].z;
            x2 = this.sprites[s].x+this.sprites[s].x_offset + this.sprites[s].width;
            y2 = this.sprites[s].y+this.sprites[s].y_offset + this.sprites[s].height - this.sprites[s].z;
            z = this.sprites[s].y+this.sprites[s].y_offset-(this.sprites[s].z/this.sprites[s].height+1);
    ...
    {
        // First look for sprites
        for(var i=0 ; i<this.sprites.length ; i++)
        {
            if ( this.sprites[i].collide_point(x, y) )
                return this.sprites[i];
        }
        
        var col = Math.min(Math.max(Math.floor(x / this.tile_size.width), 0), this.columns-1);
        var row = Math.min(Math.max(Math.floor(y / this.tile_size.height), 0), this.rows-1);
    ...
    glixl.Scene.prototype.find_path = function(start, end)
    {
        ...
        var start_row = Math.floor(((start.y+start.y_offset)-start.z) / this.tile_size.height);
  
        var end_col = Math.floor(end.x / this.tile_size.width);
        var end_row = Math.floor(((end.y+end.y_offset)-end.z) / this.tile_size.height);
        ...
        while ( !(col === end_col && row === end_row) ) 
        {
            ...
            for (var i=0 ; i<moore_neighbourhood.length ; i++) 
            {
                ...
                if ((c==end_col && r==end_row) || ( !this.tilemap[c][r][depth] && !findInClosed(c, r) ))
                {
        ...
        while(! (col === start_col && row === start_row) )
        {
            ...
            //TODO: when you click on an impassable object, the last node will be that impassable object.
            path.unshift({x: col*this.tile_size.width, y: row*this.tile_size.height});//+start.z});
    ...
    glixl.Scene.prototype.get_sprites = function(box)
    {
        var sprites = [];
        for(var i=0 ; i<this.sprites.length ; i++)
        {
            // TODO: one day, when I make flying things, this will need to check z too.
            if (this.sprites[i].x > box.x1 && this.sprites[i].x < box.x2 &&
                this.sprites[i].y > box.y1 && this.sprites[i].y < box.y2)
    ...
    glixl.Scene.prototype.get_tile_block = function(box)
    {
        var start_col = Math.floor(box.x1 / this.tile_size.width);
        var start_row = Math.floor(box.y1 / this.tile_size.height);
  
        var end_col = Math.floor(box.x2 / this.tile_size.width);
        var end_row = Math.floor(box.y2 / this.tile_size.height);
        
        var depth = Math.floor(box.z / this.tile_size.height);
        
        // Limit
        start_col = Math.min(Math.max(start_col, 0), this.columns-1);
        start_row = Math.min(Math.max(start_row, 0), this.rows-1);
        
        end_col = Math.min(Math.max(end_col, 0), this.columns-1);
        end_row = Math.min(Math.max(end_row, 0), this.rows-1);
        
        tiles = [];
        for (var r=start_row ; r<=end_row ; r++)
            for (var c=start_col ; c<=end_col ; c++)
            {
                if (this.tilemap[c][r][depth])
                    tiles.push(this.tilemap[c][r][depth]);
            }
